...
class CRM_Tools_PriceSetImporter {
  ...
  public function __construct($csvFile) {
    ...
    foreach ($this->_fileData as &$row) {
      $row['Amount Current Year'] = str_replace('$', '', $row['Amount Current Year']);
      $row['Amount Next Year'] = str_replace('$', '', $row['Amount Next Year']);
    }
  }
  ...
  public function import() {
    ...
    $toInsert = array();
    $this->createPriceSets($toInsert, TRUE, TRUE, "CIC", 'Current');
    $this->createPriceSets($toInsert, TRUE, TRUE, "CIC", 'Next');
    $this->createPriceSets($toInsert, TRUE, FALSE, "CSCT", 'Current');
    $this->createPriceSets($toInsert, TRUE, FALSE, "CSCT", 'Next');
    $this->createPriceSets($toInsert, TRUE, FALSE, "CSChE", 'Current');
    $this->createPriceSets($toInsert, TRUE, FALSE, "CSChE", 'Next');
    $this->createPriceSets($toInsert, TRUE, FALSE, "CSC", 'Current');
    $this->createPriceSets($toInsert, TRUE, FALSE, "CSC", 'Next');
    
    ksort($toInsert);
    
    foreach($toInsert as $priceSet => $baoContainer) {
      $society = $baoContainer['society'];
      $year = $baoContainer['year'];
      $priceSetId = $baoContainer['bao']->insert();
  ...
  private function createPriceSets(&$toInsert, $isMem, $isContrib, $society, $year) {
    $cnt = 0;
    foreach ($this->_fileData as $row) {
      $cnt += 10;
      ...
      if (array_key_exists('priceSet', $toInsert) ) {
        continue;
      }
      ...
      $baoContainner = array();
      $baoContainner['bao'] = $bao;
      $baoContainner['society'] = $society;
      $baoContainner['year'] = $year;
      $toInsert[$priceSet] = $baoContainner;
    }
  }
  ...
  private function createMembershipTypes($society) {
    ...
    foreach ($this->_fileData as $row) {
      ...
      $exempt = $this->getBoolean($row['Exempt']);
      ...
      $financeTypeId = CRM_Financial_DAO_FinancialType::getFieldValue('CRM_Financial_DAO_FinancialType', $account, 'id', 'description', TRUE);
      if (!$financeTypeId) {
        throw new Exception("Did not find $financeType for membershiptype $memType");
      }

      $adminOnly = $this->getBoolean($row['Admin Only?']);
      ...
      $amount = $row['Amount Current Year'];
      ...
      $bao->visibility = $adminOnly ? "Admin" : "Public";
  ...
  private function getBoolean($fieldValue) {
    if (!$fieldValue) {
      return FALSE;
    }
    $firstChar = strtolower(substr($fieldValue, 0, 1));
    return $firstChar == 'y' || $firstChar == 't' || $firstChar == '1';
  }
...
//                'tax_rate' => 8,
  ...
  private function createAccount($society, $year, $row) {
    ...
    $this->createFinancialTypeIfMissing($financeType, $accountNum);
  }
  ...
  private function createFinancialTypeIfMissing($financeType, $accountingCode) {
    $financialAccountId = CRM_Financial_DAO_FinancialAccount::getFieldValue('CRM_Financial_DAO_FinancialAccount', $accountingCode, 'id', 'accounting_code', TRUE);
    if ($financialAccountId == FALSE) {
      throw new Exception("Missing account '" . $accountingCode . ".'");
    }
    $financeTypeId = CRM_Financial_DAO_FinancialType::getFieldValue('CRM_Financial_DAO_FinancialType', $accountingCode, 'id', 'description', TRUE);
    ...
    if ($financeTypeId == FALSE) {
      ...
      $ftDAO->description = $accountingCode;
      $ftDAO->is_active = 1;
      $ftDAO->is_reserved = 0;
...
//            }
  ...
  private function createPriceSetFieldsAndOptions($society, $year, $priceSetIn, $priceSetId) {
    ...
    foreach ($this->_fileData as $row) {
      ...
      $exempt = $this->getBoolean($row['Exempt']);
      $required = $this->getBoolean($row['Mandatory']);
